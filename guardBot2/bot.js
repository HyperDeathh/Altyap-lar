...
mongoose.connect('mongodb+srv://birbenemre:194194Eb@cluster0.uuddwt6.mongodb.net/?retryWrites=true&w=majority', {useNewUrlParser: true, useUnifiedTopology: true});// Mongo connect bağlantısı.
...
// Rolü oluşturtup geri dağıtma kısmı
  if(command === "kur" || command === "kurulum" || command === "backup" || command === "setup") {
    ...
    Database.findOne({guildID: ayarlar.guildID, roleID: args[0]}, async (err, roleData) => {
      ...
      if (logKanali) { logKanali.send(new MessageEmbed().setColor("#00ffdd").setTitle('Rol Yedeği Kullanıldı!').setDescription(`${message.author} (${message.author.id}) tarafından **${roleData.name} (${roleData.roleID})** rolünün yedeği kurulmaya başlandı! Rol tekrar oluşturularak, üyelerine dağıtılmaya ve izinleri kanallara eklenmeye başlanıyor.`).setFooter('Lunagram Guard').setTimestamp()).catch(); } else { message.guild.owner.send(new MessageEmbed().setColor("#00ffdd").setTitle('Rol Yedeği Kullanıldı!').setDescription(`${message.author} (${message.author.id}) tarafından **${roleData.name} (${roleData.roleID})** rolünün yedeği kurulmaya başlandı! Rol tekrar oluşturularak, üyelerine dağıtılmaya ve izinleri kanallara eklenmeye başlanıyor.`).setFooter('Lunagram Guard').setTimestamp()).catch(err => {}); };
    });
  };
});
...
client.on("guildMemberUpdate", async (oldMember, newMember) => {
  if (newMember.roles.cache.size > oldMember.roles.cache.size) {
    ...
    if (yetkiPermleri.some(p => !oldMember.hasPermission(p) && newMember.hasPermission(p))) {
      ...
      let logKanali = client.channels.cache.get(ayarlar.logChannelID);
      if (logKanali) { logKanali.send(new MessageEmbed().setColor("#00ffdd").setTitle('Sağ Tık Yetki Verildi!').setDescription(`${newMember} (${newMember.id}) üyesine ${entry.executor} (${entry.executor.id}) tarafından sağ tık yetki verildi! Veren kişi yasaklandı ve verilen kişiden rol geri alındı.`).setFooter('Lunagram Guard').setTimestamp()).catch(); } else { newMember.guild.owner.send(new MessageEmbed().setColor("#00ffdd").setTitle('Sağ Tık Yetki Verildi!').setDescription(`${newMember} (${newMember.id}) üyesine ${entry.executor} (${entry.executor.id}) tarafından sağ tık yetki verildi! Veren kişi yasaklandı ve verilen kişiden rol geri alındı.`).setFooter('Lunagram Guard').setTimestamp()).catch(err => {}); };
    };
  };
});
// Rol açtırmama
client.on("roleCreate", async role => {
  ...
  if (!entry || !entry.executor || Date.now()-entry.createdTimestamp > 5000 || guvenli(entry.executor.id) || !ayarlar.roleGuard) return;
  role.delete({ reason: "Lunagram Rol Koruma" });
  cezalandir(entry.executor.id, "jail");
  let logKanali = client.channels.cache.get(ayarlar.logChannelID);
  if (logKanali) { logKanali.send(new MessageEmbed().setColor("#00ffdd").setTitle('Rol Oluşturuldu!').setDescription(`${entry.executor} (${entry.executor.id}) tarafından bir rol oluşturuldu! Oluşturan kişi jaile atıldı ve rol silindi.`).setFooter('Lunagram Guard').setTimestamp()).catch(); } else { role.guild.owner.send(new MessageEmbed().setColor("#00ffdd").setTitle('Rol Oluşturuldu!').setDescription(`${entry.executor} (${entry.executor.id}) tarafından bir rol oluşturuldu! Oluşturan kişi jaile atıldı ve rol silindi.`).setFooter('Lunagram Guard').setTimestamp()).catch(err => {}); };
});
...
function ytKapat(guildID) {
  ...
  let logKanali = client.channels.cache.get(ayarlar.logChannelID);
  if (logKanali) { logKanali.send(new MessageEmbed().setColor("#00ffdd").setTitle('İzinler Kapatıldı!').setDescription(`Rollerin yetkileri kapatıldı!`).setFooter('Lunagram Guard').setTimestamp()).catch(); } else { logKanali.guild.owner.send(new MessageEmbed().setColor("#00ffdd").setTitle('İzinler Kapatıldı!').setDescription(`Rollerin yetkileri kapatıldı!`).setFooter('Lunagram Guard').setTimestamp()).catch(err => {}); };
};
